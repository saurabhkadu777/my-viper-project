...
def _parse_cvrf_doc(cvrf_data: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        document_tracking_id = document_tracking.get('Identification', {}).get('ID', '') if isinstance(document_tracking, dict) else ''
...
def fetch_patch_tuesday_data(year: int, month: int) -> Optional[List[Dict[str, Any]]]:
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Direct CVRF document fetch failed: {str(e)}. Trying to find document ID from updates list.")
