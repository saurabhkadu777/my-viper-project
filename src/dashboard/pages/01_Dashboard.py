...
# EPSS Score range
epss_range = st.sidebar.slider("EPSS Score Range", min_value=0.0, max_value=1.0, value=(0.0, 1.0), step=0.01)
epss_min, epss_max = epss_range

# KEV filter
kev_filter = st.sidebar.radio("CISA KEV Status", options=["All", "Only KEV", "Exclude KEV"], index=0)
...
exploit_filter = st.sidebar.radio(
    "Public Exploit Status", options=["All", "Only with Exploits", "Exclude Exploits"], index=0
)
...
ms_severity_filter = st.sidebar.selectbox(
    "Microsoft Severity", options=["All", "Critical", "Important", "Moderate", "Low"], index=0
)
...
with metrics_cols[2]:
    ...
    st.metric(
        "In CISA KEV",
        kev_count,
        delta=f"{kev_count/len(df)*100:.1f}%" if len(df) > 0 else "0%",
        delta_color="off",
    )
...
with metrics_cols[3]:
    ...
    st.metric(
        "Public Exploits",
        exploit_count,
        delta=f"{exploit_count/len(df)*100:.1f}%" if len(df) > 0 else "0%",
        delta_color="off",
    )
...
with priority_cols[1]:
    ...
    exploit_status = (
        df["has_public_exploit"].map({True: "Has Exploit", False: "No Exploit"}).value_counts().reset_index()
    )
...
with priority_cols[2]:
    ...
    if not df_date_valid.empty:
        ...
        fig_timeline = px.line(df_by_date, x="Date", y="CVEs", markers=True, title="CVEs Over Time")
...
with risk_cols[0]:
    ...
    if not kev_risks.empty:
        fig_risk.add_trace(go.Box(y=kev_risks, name="KEV", marker_color="#ff4b4b"))

    if not non_kev_risks.empty:
        fig_risk.add_trace(go.Box(y=non_kev_risks, name="Non-KEV", marker_color="#4b9fff"))

    fig_risk.update_layout(title="Risk Score Distribution by KEV Status", yaxis_title="Risk Score", height=300)
...
if not kev_entries.empty and "kev_date_added" in kev_entries.columns:
    ...
    if not recent_kev.empty:
        ...
        recent_kev_display = recent_kev_display[
            ["cve_id", "kev_date_added", "gemini_priority", "cvss_v3_score", "description"]
        ]

        # Format numbers
        if "cvss_v3_score" in recent_kev_display.columns:
            recent_kev_display["cvss_v3_score"] = recent_kev_display["cvss_v3_score"].apply(
...
if not ms_critical.empty:
    ...
    if not recent_critical.empty:
        ...
        # Only select relevant columns
        ms_critical_display = ms_critical_display[
            [
                "cve_id",
                "microsoft_severity",
        ...
        if "cvss_v3_score" in ms_critical_display.columns:
            ms_critical_display["cvss_v3_score"] = ms_critical_display["cvss_v3_score"].apply(
                lambda x: f"{x:.1f}" if pd.notnull(x) else "N/A"
            )

        # Rename columns for display
        ms_critical_display.columns = [
            "CVE ID",
            "MS Severity",
            "MS Product",
            "Patch Tuesday",
...
display_df["KEV Status"] = display_df.apply(
    lambda row: f"🚨 Yes ({row['kev_date_added']})"
    if row["is_in_kev"] and pd.notnull(row["kev_date_added"])
    else "🚨 Yes"
    if row["is_in_kev"]
    else "No",
    axis=1,
...
display_df["MS Info"] = display_df.apply(
    ...
    else row["microsoft_severity"]
    if pd.notnull(row["microsoft_severity"])
    else "N/A",
    axis=1,
)
...
display_df = display_df[cols_to_display].rename(
    columns={
        "cve_id": "CVE ID",
...
if selected_cve:
    ...
    kev_badge = (
        '<span style="background-color: #ff4b4b; color: white; padding: 3px 8px; border-radius: 3px; font-weight: bold; margin-left: 10px;">CISA KEV</span>'
        if is_kev
        else ""
    )
    ...
    with detail_cols[0]:
        st.markdown(
            f"**Published:** {selected_data.get('published_date').strftime('%Y-%m-%d') if pd.notnull(selected_data.get('published_date')) else 'N/A'}"
        )
    ...
    with detail_cols[2]:
        ...
        if is_kev and not any("CISA KEV" in alert for alert in alerts):
            st.error(
                "**ALERT:** This vulnerability is in the CISA Known Exploited Vulnerabilities (KEV) catalog, indicating it is being actively exploited in the wild."
            )
...
if not top_cvss.empty:
    ...
    formatted_top_cvss["risk_score"] = formatted_top_cvss["risk_score"].apply(
    ...
    st.markdown(formatted_top_cvss.to_html(escape=False, index=False), unsafe_allow_html=True)
...
if not exploitable_cves.empty:
    ...
    with exploit_cols[0]:
        ...
        st.metric(
            "High Priority & Exploitable",
            high_priority_exploitable,
    ...
    with exploit_cols[1]:
        ...
        # Format the table
        formatted_exploitable = top_exploitable[
            ["cve_id", "cvss_v3_score", "gemini_priority", "risk_score", "is_in_kev"]
        ].copy()
        ...
        formatted_exploitable["risk_score"] = formatted_exploitable["risk_score"].apply(
            lambda x: f"{x:.2f}" if pd.notnull(x) else "N/A"
        )
        ...
        # Display with HTML formatting to enable clickable links
        st.markdown(formatted_exploitable.to_html(escape=False, index=False), unsafe_allow_html=True)
            ...
            try:
                ...
                if exploits and len(exploits) > 0:
                    ...
                    with st.sidebar.expander("View Results", expanded=True):
                        ...
                        if len(exploits) > 5:
                            st.info(
                                f"{len(exploits) - 5} more results not shown. Visit Detailed Analysis for full results."
                            )
                else:
