...
def calculate_combined_risk_score(cve_data: Dict) -> float:
    ...
    try:
        ...
        combined_score = (
            w_gemini * priority_factor + w_cvss * cvss_normalized + w_epss * epss_normalized + w_ms * ms_factor
        )
        ...
        if has_public_exploit:
            ...
            logger.info(
                f"Applied public exploit boost to {cve_id}, increasing score from {prev_score:.4f} to {combined_score:.4f}"
            )
...
def generate_alerts(cve_data: Dict) -> List[str]:
    ...
    try:
        ...
        if epss_score is not None and epss_score >= rules["critical_epss"]:
            alerts.append(
                f"CRITICAL EXPLOITABILITY: {cve_id} has a {epss_score:.2%} probability of exploitation (threshold: {rules['critical_epss']:.2%})"
            )

        # Rule 2: Severe Impact & Likely Exploit
        if (
            cvss_score is not None
            and cvss_score >= rules["severe_cvss"]
            and epss_score is not None
            and epss_score >= rules["severe_epss"]
        ):
            alerts.append(
                f"SEVERE IMPACT & LIKELY EXPLOIT: {cve_id} has a high CVSS score ({cvss_score}) and significant exploitation probability ({epss_score:.2%})"
            )
        ...
        if epss_score is not None and epss_score >= rules["high_impact_epss"]:
            ...
            for keyword in rules["high_impact_keywords"]:
                if keyword in description:
                    alerts.append(
                        f"HIGH IMPACT TECHNIQUE: {cve_id} matches keyword '{keyword}' and has significant exploitation probability ({epss_score:.2%})"
                    )
                    break
        ...
        if is_in_kev:
            ...
            alerts.append(
                f"KNOWN EXPLOITED: {cve_id} is in CISA's Known Exploited Vulnerabilities catalog (added{added_info})"
            )
